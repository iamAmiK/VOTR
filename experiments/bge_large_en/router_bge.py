...
from experiments.bge_large_en.embedder_bge import BgeEmbedder
...
def _default_cfg() -> RouterConfig:
    ...
    bge_cfg = root / "experiments" / "bge_large_en" / "config.bge.yaml"
    return load_config(base_path=bge_cfg)
...
def create_app(cfg: Optional[RouterConfig] = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        embedder = BgeEmbedder(model_name=cfg.embedding_model)
    ...
    app = FastAPI(title="MCP-Router (BGE Large EN)", version="0.1.0", lifespan=lifespan)
