"""BGE one-shot benchmark runner.

Starts BGE router on port 8776, runs all functional correctness suites,
and writes reports under benchmarks/results/functional_correctness_bge_large_en.
"""
...
OUT = ROOT / "benchmarks" / "results" / "functional_correctness_bge_large_en"
EVAL = FC / "evaluate.py"
...
def main() -> None:
    ...
    proc = subprocess.Popen(
        [
            ...
            "-m",
            "uvicorn",
            "experiments.bge_large_en.router_bge:app",
            "--host",
            "127.0.0.1",
            "--port",
            str(PORT),
    ...
    try:
        for _ in range(90):
            ...
            raise RuntimeError(f"BGE router did not become healthy in time.\n{err}")
        ...
        print(f"\nWrote BGE reports under {OUT}", flush=True)
    finally:
        proc.terminate()
        try:
