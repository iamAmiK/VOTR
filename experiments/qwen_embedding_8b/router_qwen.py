...
from experiments.qwen_embedding_8b.embedder_qwen import QwenEmbedder
...
def _default_cfg() -> RouterConfig:
    ...
    qwen_cfg = root / "experiments" / "qwen_embedding_8b" / "config.qwen.yaml"
    return load_config(base_path=qwen_cfg)
...
def create_app(cfg: Optional[RouterConfig] = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        embedder = QwenEmbedder(model_name=cfg.embedding_model, dimensions=cfg.embedding_dimensions)
    ...
    app = FastAPI(title="MCP-Router (Qwen Embedding 8B)", version="0.1.0", lifespan=lifespan)
