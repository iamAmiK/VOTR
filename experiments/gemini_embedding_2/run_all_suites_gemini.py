"""Gemini embedding one-shot benchmark runner.

Starts Gemini router on port 8777, runs all functional correctness suites,
and writes reports under benchmarks/results/functional_correctness_gemini_embedding_2.
"""
...
def main() -> None:
    ...
    try:
        ...
        print(f"\nWrote Gemini reports under {OUT}", flush=True)
    finally:
        proc.terminate()
        try:
